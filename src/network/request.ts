...
import { getUUID } from "@/tool/global";
import {
  AxiosRequestConfig,
  AxiosResponse,
  AxiosError,
  ...
  private initInterceptors() {
    ...
    this.axiosInstance.interceptors.request.use(
      (config: AxiosRequestConfig) => {
        ...
        const token: string | null = localStorage.getItem(localSessionKey);
        if (token && config.headers) {
          config.headers["x-tt-session-v2"] = token;
        } else {
          localStorage.set(localSessionKey, getUUID());
          // eslint-disable-next-line @typescript-eslint/ban-ts-comment
          // @ts-ignore
          config.headers["x-tt-session-v2"] = getUUID();
          alert("获取token失败！");
        }
